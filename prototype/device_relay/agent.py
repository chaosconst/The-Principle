...
class GenesisWorker:
    ...
    async def run_loop(self, loop_was_running=False):
        ...
        last_cfh = max(c_stripped.rfind('/call_for_human'), c_stripped.rfind('/call_for_trigger'), c_stripped.rfind('/sleep'))
    ...
    async def loop(self):
        if not self.pending_user_input and ('/call_for_human' in self.consciousness or '/call_for_trigger' in self.consciousness or '/sleep' in self.consciousness):
            last_sc = self.consciousness.rfind('/self_continue')
            last_cfh = max(self.consciousness.rfind('/call_for_human'), self.consciousness.rfind('/call_for_trigger'), self.consciousness.rfind('/sleep'))
            last_trigger = self.consciousness.rfind('System - [Trigger]')
            if last_cfh > last_sc and last_cfh > last_trigger:
                self._log(f"[{ts()}] [infero] loop(): /sleep at end, no trigger after it, returning")
                return
        ...
        while self.running:
            ...
            last_cfh = max(B_stripped.rfind('/call_for_human'), B_stripped.rfind('/call_for_trigger'), B_stripped.rfind('/sleep'))
    ...
    def _build_realtime(self):
        ...
        lines += '\n      async def loop(): await perceive(); B = await infer(); await act(B); if /self_continue in B: repeat; if /sleep: sleep'
        ...
        lines += f'\n    - Trigger: echo "msg" >> {INFERO_DIR}/beings/{self.being_id}/trigger.txt to wake from /sleep. Use in nohup scripts for async callback. E.g.: nohup bash -c \'sleep 3600 && echo "1h timer" >> {INFERO_DIR}/beings/{self.being_id}/trigger.txt\' &'
        lines += '\n    - Watchdog: if no trigger for 30 min after /sleep, auto-wakes with [watchdog] message.'
        # Other devices
        for name, info in self.devices.items():
            ...
            if dtype == 'browser':
                ...
                lines += '\n      (30s timeout: process keeps running but stdout/stderr detached, loop advances. Write output to file if needed after 30s.)'
        return f'[Realtime]\nReminder: end with /self_continue or /sleep\nDevices:{lines}'
